...
from clean_df_all import df_out_sample_set_weekly, df_out_sample_set_daily
...
# === Run backtest
print("\n=== Running Panel B strategy: psn-ADCC-Skewed t Copula (Long Only) ===")
...
if isinstance(perf_series, pd.Series) and not perf_series.empty and pd.api.types.is_numeric_dtype(perf_series):
    ...
    #     title="Panel B: psn-ADCC-Skewed t Copula (Long Only)",
    #     figsize=(10, 5)
    # )
    # plt.ylabel("Cumulative Return")
    # plt.xlabel("Date")
    # plt.tight_layout()
    # plt.savefig("psn_adcc_skt_cumulative_return.png")
    # plt.show()

    # perf_series.to_csv("psn_adcc_skt_returns.csv")
    ...
    print("\n=== Panel B: psn-ADCC-SKT Performance ===")
