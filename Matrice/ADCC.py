...
class ADCC_Portfolio:
    ...
    def calculate_adcc_matrices(self, standardized_residuals, conditional_variances, alpha=None, beta=None, gamma=None):
        """Calculer la matrice de corrélation et covariance ADCC pour un ensemble de données"""
        if alpha is None:
            alpha = self.alpha
        if beta is None:
            beta = self.beta
    ...
    def fit_in_sample_adcc(self):
        ...
        R_t, H_t = self.calculate_adcc_matrices(residuals, variances)
    ...
    def calculate_out_sample_matrices(self):
        ...
        for i, target_date in enumerate(out_sample_dates):
            ...
            # Calculer les matrices avec les paramètres ADCC fixes
            R_t, H_t = self.calculate_adcc_matrices(residuals, variances, self.alpha, self.beta, self.gamma)
    ...
    def export_results(self, base_filename='adcc'):
